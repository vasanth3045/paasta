...
This file stores periodically scheduled jobs for execution on `Tron <https://github.com/yelp/tron>`_.
...
Each Tron **job** configuration MAY specify the following options:
  ...
  * ``service``: Configures PaaSTA to use the docker image from an alternative service.
    Defaults to the name of the directory that the Tron config file resides in.
  ...
  * ``executor``: Configures Tron to execute the command in a particular way.
    ...
    Defauls to ``ssh``, which is he classic Tron execution method.
...
Each Tron **action** of a job MAY specify the following:
  ...
  * ``executor``: Same setting as the ``Job``, but on a per-action basis. Defaults
    to the setting for the entire job.
